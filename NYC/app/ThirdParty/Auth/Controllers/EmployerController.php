...
class EmployerController extends Controller
{
	...
	public function home()
	{
		...
		return view($this->config->views['employer'], ['config' => $this->config]);
	}
	...
	public function register()
	{
		...
		if ($this->request->getMethod() === 'post'){
			...
			if ($this->validate($rules)) {
				...
				$data['password_hash'] = password_hash($this->request->getPost('confirm_password'),PASSWORD_DEFAULT);

				$data['created_at'] = time();
				$data['group_id'] = 2;
				$data['active'] = 1;
				$this->session = \Config\Services::session(); 
				if($this->RegisterModel->add($data)){
					$this->session->setFlashdata('success', 'Sign Up successfully.');
				}
			return	redirect('employer', 'refresh');
			}else{
				return view($this->config->views['employer'], ['validation' => $this->validator,'reset'=>false]);
			}
		}
		$data['reset'] = true;
		return view($this->config->views['employer'], $data);
	}
	...
	public function empLogin()
	{
		...
		return view($this->config->views['employer-signIn'], ['config' => $this->config]);
	}

	public function attemptEmpLogin()
	{
		$rules = [
			'email'		=> 'required',
			'password' 	=> 'required|min_length[5]',
		...
		$emp = $this->RegisterModel->where('email', $this->request->getPost('email'))->get()->getRowArray();
		if (is_null($emp) ||
			! password_verify($this->request->getPost('password'), $emp['password_hash'])
		) {
			return redirect()->to('employer-signIn')->withInput()->with('error', lang('Auth.wrongCredentials'));
		}

		

		// login OK, save user data to session
		$this->session->set('isEmpLoggedIn', true);
		$this->session->set('empData', [
			'id' 			=> $emp['id'],
	
		]);
		return redirect()->to('/employer/student-lists');
