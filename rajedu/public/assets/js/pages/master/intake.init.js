...
var masterIntake = function() {
    ...
    var handleIntakeListDT = function (e) {
        datatable = $(table).DataTable({
            ...
            'ajax': {
                url: site_url+'master/get-intake-dt',
                data: function(data){
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach(ed => {
            ed.addEventListener('click', function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: site_url + 'master/get-intake-details',
                    data: { id: $(ed).attr('data-id'), x_token: csrf.val()},
                    success: function(data) {
                        if (data.success) {
                            csrf.val(data.hash);
                            $('#status').val(data['result'].status).trigger('change');
                            $('#intake_prefix').val(data['result'].intake_prefix).trigger('change');
                            $('#intake_year').val(data['result'].intake_year).trigger('change');
            ...
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: site_url + 'master/delete-intake',
            ...
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: site_url + 'master/change-intake-status',
    ...
    return {
        ...
        init: function() {
            form = $('#form-intake');
            submitButton = $('#btn-intake');
            btnHtml = submitButton.html();
            csrf = $('[name=x_token]');
            table = document.querySelector('#dt-intake');
            modalEl = document.querySelector('#modal-master-intake');
            if (table) {
                handleIntakeListDT();
            }
