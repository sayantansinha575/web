...
var masterSearchTags = function() {
    ...
    var handleStatusListDT = function (e) {
        datatable = $(table).DataTable({
            ...
            'ajax': {
                url: site_url+'master/get-search-dt',
                data: function(data){
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach(ed => {
            ed.addEventListener('click', function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: site_url + 'master/get-search-details',
                    data: { id: $(ed).attr('data-id'), x_token: csrf.val()},
                    success: function(data) {
                        if (data.success) {
                            csrf.val(data.hash);
                            $('#status').val(data['result'].status).trigger('change');
                            $('#search-tag').val(data['result'].search_tags);
                            $('#program-name').val(data['result'].title);
            ...
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: site_url + 'master/delete-search',
            ...
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: site_url + 'master/change-search-status',
    ...
    return {
        ...
        init: function() {
            ...
            submitButton = $('#btn-search-tag');
            btnHtml = submitButton.html();
            csrf = $('[name=x_token]');
            table = document.querySelector('#dt-search-tag');
            modalEl = document.querySelector('#modal-search-tag');
            if (table) {
                handleStatusListDT();
            }
